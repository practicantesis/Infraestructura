...
$conn = new mysqli('syslog', 'rsyslog', 'logsys', 'Syslog');
...
$html = "<table border = 1><tr><th>OFI</th><th>STATUS</th><th>ACTIVO DESDE</th><th>ULTIMO CONTACTO</th><tr>";
    ...
    $sql = "select * from SystemEvents where FromHost != 'syslogs'";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        while($value = $result->fetch_assoc()) {
            $html .= "<tr><td>".$value['ReceivedAt']."</td><td>".$value['FromHost']."</td><td><small>".$value['Message']."</small></td><td><small>".$value['Priority']."</small></td><tr>";
        }
    }
